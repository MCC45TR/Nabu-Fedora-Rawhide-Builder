...
- `nabu-kde-color-profiles`: local, license-safe QDCM-to-ICC conversion and
  KScreen selection tooling;
...
on AArch64; the resulting packages from this source are noarch.
...
panel discovery and `auto` subcommand remain available only as an explicit
diagnostic or administrator action; the package never invokes them
automatically.
...
`senemos-nabu-color-profile` remains available to validate or regenerate the
profiles from an authorized local source:

    senemos-nabu-color-profile catalog
    senemos-nabu-color-profile auto --dry-run  # explicit diagnostic only
    senemos-nabu-color-profile import /authorized/path/qdcm_calib_data_xiaomi_36_02_0b_video_mode_dual_dsi_cphy_panel.xml
    senemos-nabu-color-profile validate ~/.local/share/icc/senemos/nabu/xiaomi-nabu-36-02-0b-srgb.icc
    senemos-nabu-color-profile apply srgb --panel 36-02-0b --dry-run
    senemos-nabu-color-profile apply srgb --panel 36-02-0b

The importer recognizes the `36_02_0b` and `42_02_0a` panel variants and
keeps their output files separate. It converts only Android's static SDR
