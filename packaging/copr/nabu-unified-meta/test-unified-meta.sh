...
bash -n "$root/nabu" "$root/nabu-kernel-maintenance" "$root/nabu-kernel-offline-finalize" "$root/build-srpms.sh" "$root/test-kernel-maintenance-family.sh" "$root/../nabu-kernel-maintenance/nabu-maintenance-ready"
bash "$root/test-kernel-maintenance-family.sh"
bash "$root/test-offline-kernel-finalize.sh"
bash "$root/../nabu-kernel-maintenance/test-maintenance-ready.sh"
...
fastrpc_restart="$root/vendor-src/nabu-system-integration-2.0.0/runtime/20-nabu-fastrpc-restart.conf"
grep -Fxq 'StartLimitIntervalSec=60s' "$fastrpc_restart" || fail "ADSP FastRPC restart window is not bounded"
grep -Fxq 'StartLimitBurst=4' "$fastrpc_restart" || fail "ADSP FastRPC restart burst is not bounded"
grep -Fxq 'TimeoutStopFailureMode=kill' "$fastrpc_restart" || fail "ADSP FastRPC retains the doubled abort timeout"
...
grep -Fq 'nabu-kernel-maintenance-api = 7' "$core" || fail "maintenance API"
...
grep -Fq 'prepared_uki_digest =~ ^[0-9a-f]{64}$' "$root/nabu-kernel-maintenance" || fail "per-family prepared UKI digest gate missing"
grep -Fq 'prepared_kernel_digest == "$kernel_digest"' "$root/nabu-kernel-maintenance" || fail "prepared kernel payload digest gate missing"
grep -Fq 'prepared_dtb_digest == "$dtb_digest"' "$root/nabu-kernel-maintenance" || fail "prepared Nabu DTB digest gate missing"
...
grep -Fq 'ExecCondition=/usr/libexec/senemos-nabu/nabu-maintenance-ready' "$root/../nabu-kernel-maintenance/nabu-kernel-maintenance.service" || fail "idle maintenance readiness gate missing"
grep -Fq 'ExecStartPre=-/usr/libexec/nabu-refind-sync' "$root/../nabu-kernel-maintenance/nabu-kernel-maintenance.service" || fail "rEFInd synchronization is not folded into maintenance"
grep -Fq 'OnActiveSec=30min' "$root/../nabu-kernel-maintenance/nabu-kernel-maintenance.timer" || fail "maintenance still starts in the boot path"
grep -Fq 'OnUnitInactiveSec=6h' "$root/../nabu-kernel-maintenance/nabu-kernel-maintenance.timer" || fail "deferred maintenance retry cadence missing"
grep -Fq 'Persistent=false' "$root/../nabu-kernel-maintenance/nabu-kernel-maintenance.timer" || fail "missed maintenance would run during boot"
[[ ! -e "$root/../nabu-kernel-maintenance/nabu-kernel-maintenance.path" ]] || fail "kernel path watcher still shipped"
...
grep -Fq '"$refind_sync"' "$root/nabu-kernel-offline-finalize" || fail "offline boot finalizer does not invoke rEFInd sync"
...
! sed '/^%changelog/,$d' "$root/nabu-core-meta.spec" | grep -Eq '^Source[0-9]+:.*nabu-locale-packages' || fail "runtime locale installer remains in CORE"
! sed '/^%changelog/,$d' "$root/nabu-core-meta.spec" | grep -Eq '^%\{_unitdir\}/nabu-locale-packages' || fail "runtime locale units remain packaged"
grep -Fq 'disable --now \' "$root/nabu-core-meta.spec" || fail "removed first-boot services are not disabled on upgrade"
grep -Fq 'nabu-locale-packages.path nabu-locale-packages.timer' "$root/nabu-core-meta.spec" || fail "legacy locale units are not retired on upgrade"
grep -Fxq 'hostonly="yes"' "$root/vendor-src/nabu-system-integration-2.0.0/payload/etc/dracut.conf.d/99-nabu-generic.conf" || fail "Nabu initramfs remains generic"
[[ ! -e "$root/vendor-src/nabu-system-integration-2.0.0/runtime/nabu-pmic-rtc-sync.service" ]] || fail "redundant PMIC RTC boot service remains"
[[ ! -e "$root/vendor-src/nabu-system-integration-2.0.0/runtime/nabu-root-growfs.service" ]] || fail "duplicate root-grow service remains"
wifi_policy="$root/vendor-src/nabu-system-integration-2.0.0/runtime/20-nabu-wifi-wowlan.conf"
grep -Fxq '[connection-nabu-wifi]' "$wifi_policy" || fail "Nabu Wi-Fi connection policy section missing"
grep -Fxq 'match-device=type:wifi' "$wifi_policy" || fail "Nabu Wi-Fi policy is not scoped to Wi-Fi devices"
grep -Fxq 'wifi.wake-on-wlan=12' "$wifi_policy" || fail "Nabu selective Wi-Fi wake policy missing"
...
grep -Fqx 'Requires:       nabu-hardware-provenance >= 1.0.0-3' "$core" \
