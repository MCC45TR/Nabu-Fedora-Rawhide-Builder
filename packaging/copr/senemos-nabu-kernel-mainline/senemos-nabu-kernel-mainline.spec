...
Source3:        91-nabu-mainline-omit-early-xhci.conf
...
if [ ! -e "$marker" ]; then
    /usr/bin/systemctl preset nabu-mainline-late-xhci.service >/dev/null 2>&1 || :
...
temporary=$(mktemp /var/lib/nabu-kernel-maintenance/pending.d/.mainline.XXXXXX)
printf '%%s\n' '%{uname_r}' > "$temporary"
chmod 0644 "$temporary"
mv -f "$temporary" /var/lib/nabu-kernel-maintenance/pending.d/mainline

%preun
%systemd_preun nabu-mainline-late-xhci.service

%postun
%systemd_postun nabu-mainline-late-xhci.service
if [ "$1" -eq 0 ]; then
    /usr/sbin/depmod -a || :
fi
