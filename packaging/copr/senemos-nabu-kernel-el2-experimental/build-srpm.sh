...
install -m0644 "$base/upstream.sha256" "$base/patches.sha256" \
    "$top/SOURCES/"
install -m0644 "$base"/patches/*.patch "$top/SOURCES/"
install -m0644 "$root/nabu-el2-experimental.config" \
    "$root/nabu-currentel.c" "$root/nabu-el2-status" "$top/SOURCES/"
