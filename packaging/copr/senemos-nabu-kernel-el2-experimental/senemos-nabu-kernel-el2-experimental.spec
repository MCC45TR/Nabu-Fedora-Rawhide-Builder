...
Version:        7.2.4
Release:        1%{?dist}
Summary:        EL2/KVM experiment kernel for Xiaomi Pad 5
...
Source3:        nabu-build-stamp
Source4:        nabu-el2-experimental.config
Source5:        nabu-currentel.c
Source6:        nabu-el2-status
...
Patch0022:      0022-senemos-isolate-mainline-kernel-identity.patch
...
Requires:       senemos-nabu-kernel-mainline >= 7.2.4-12
Provides:       kernel-nabu-el2-experimental-uname-r
Requires(postun): kmod

%description
The checksum-locked SENEMOS Linux 7.2.4 Nabu source with a distinct kernel
ABI and only the kernel-side KVM prerequisite enabled. Installing this package
does not alter the EFI System Partition, the boot default, Android, boot/HYP
partitions, or firmware. The production mainline package is a required,
co-installed fallback.

KVM can initialize only if the firmware enters Linux with EL2 available. The
included read-only UEFI CurrentEL probe and userspace status command measure
that prerequisite; neither can promote EL1 to EL2.

%prep
[[ '%{nabu_build_stamp}' =~ ^[0-9]{10}$ ]]
grep -Fxq '%{nabu_build_stamp}' %{SOURCE3}
...
# Cross only the production KVM exclusion, and keep a distinct ABI.  Merge
# this fragment last so device pruning cannot silently remove it.
KCONFIG_CONFIG=.config scripts/kconfig/merge_config.sh -m -r \
    .config %{SOURCE4}
...
# Build a storage- and variable-write-free UEFI application. It reads only
# CurrentEL, displays the result, and returns to the invoking boot manager.
clang --target=aarch64-unknown-windows -ffreestanding -fshort-wchar \
    -fno-stack-protector -fno-builtin -Wall -Wextra -Werror -Os \
    -c %{SOURCE5} -o nabu-currentel.obj
lld-link /subsystem:efi_application /entry:efi_main /machine:arm64 \
    /nodefaultlib /opt:ref /opt:icf /timestamp:0 \
    /out:nabu-currentel.efi \
    nabu-currentel.obj
...
install -Dm0755 %{SOURCE6} %{buildroot}%{_bindir}/nabu-el2-status
install -Dm0644 nabu-currentel.efi \
    %{buildroot}%{_prefix}/lib/senemos-nabu/el2/CurrentEL.efi
...
grep -Fxq '%{nabu_build_stamp}' \
    %{buildroot}%{_prefix}/lib/senemos-nabu/uki-version.d/%{uname_r}
test -s %{buildroot}%{_prefix}/lib/senemos-nabu/el2/CurrentEL.efi
test -x %{buildroot}%{_bindir}/nabu-el2-status
llvm-readobj --file-headers \
    %{buildroot}%{_prefix}/lib/senemos-nabu/el2/CurrentEL.efi \
    | grep -Fq 'Format: COFF-ARM64'
grep -Fq 'CurrentEL' %{SOURCE5}
...
grep -Fxq 'CONFIG_VIRTUALIZATION=y' %{buildroot}/boot/config-%{uname_r}
grep -Fxq 'CONFIG_KVM=y' %{buildroot}/boot/config-%{uname_r}
grep -Fxq '# CONFIG_NVHE_EL2_DEBUG is not set' \
    %{buildroot}/boot/config-%{uname_r}
grep -Fxq '# CONFIG_PTDUMP_STAGE2_DEBUGFS is not set' \
    %{buildroot}/boot/config-%{uname_r}
! grep -Eq '^CONFIG_(MODULE_SIG_FORCE|TCG_FTPM_TEE|IMA|EVM)=(y|m)$' \
    %{buildroot}/boot/config-%{uname_r}
...
%{_prefix}/lib/senemos-nabu/el2/CurrentEL.efi
%{_bindir}/nabu-el2-status

%changelog
* Sun Sep 13 2026 mcc45tr <mcc45tr@gmail.com> - 7.2.4-1
- Add a separately named, co-installable Nabu KVM prerequisite kernel while
  requiring the production 7.2.4 kernel as the known-good fallback.
- Keep install transactions away from the ESP and boot default; ship only a
  read-only CurrentEL probe and a read-only Linux status command.
- Preserve the production hardening baseline and keep EL2 debug interfaces
  disabled.
...
- Restore the six firmware-owned bonded-DSI byte/pixel branches to
  ...
  the generic disable-unused pass.
- Retain the bounded Nabu OV8856 cold-probe recovery from release 9.
...
- Describe SM8150 PRNG-EE without falsely crediting it as trusted entropy.
