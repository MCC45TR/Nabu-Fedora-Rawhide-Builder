...
check 'locale regression and driver stack have hard gates' bash -c 'grep -Fq "locale-rpm-files.txt" "$1" && grep -Fq "FILESTATES" "$1" && grep -Fq "state == \"0\"" "$1" && grep -Fq "RPM-owned locale files are missing" "$1" && grep -Fq "glibc-all-langpacks" "$1" && grep -Fq "nabu-camera-support iris-vaapi-nabu iio-sensor-proxy-nabu libssc-nabu python3-ssc-nabu" "$1"' _ "$COMPOSE"
