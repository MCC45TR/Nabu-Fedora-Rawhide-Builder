...
check 'DE locks root and masks CDC while keeping onboarding userless' bash -c 'grep -Fq "usermod -L root" "$1" && grep -Fq "mask nabu-esp32-cdc-log.service" "$1" && grep -Fq "regular-users.txt" "$1" && grep -Fq ".unconfigured" "$1" && grep -Fq "plasma-login-manager.service" "$1"' _ "$COMPOSE"
check 'locale regression and driver stack have hard gates' bash -c 'grep -Fq "locale-rpm-files.txt" "$1" && grep -Fq "RPM-owned locale files are missing" "$1" && grep -Fq "glibc-all-langpacks" "$1" && grep -Fq "nabu-camera-support iris-vaapi-nabu nabu-sensor-config nabu-sensor-monitor" "$1"' _ "$COMPOSE"
