...
check 'final image verifies root lock and complete RPM ownership map without FUSE chroot' bash -c 'grep -Fq "core_verify_root_locked_shadow \"\$mount_dir\"" "$1" && ! grep -Fq "chroot \"\$mount_dir\"" "$1" && grep -Fq "final-rpm-ownership.txt" "$1" && grep -Fq "Final KDE /etc ownership is not root:root" "$1" && grep -Fq "Final KDE /usr ownership is not root:root" "$1"' _ "$HOST"
check 'RPM ownership manifest writes real newline-delimited records' bash -c 'grep -Fq '\''f.write(f"{path}|{uid}|{gid}\n")'\'' "$1" && ! grep -Fq '\''f.write(f"{path}|{uid}|{gid}\\n")'\'' "$1"' _ "$COMPOSE"
check 'interrupted KDE builds resume from validated compose or finalized EXT4' bash -c 'grep -Fq -- "--resume-artifact" "$1" && grep -Fq "resume_finalized=1" "$1" && grep -Fq "KDE resume ownership manifest is missing" "$1" && grep -Fq "KDE resume work mount is still active" "$1" && grep -Fq "KDE resume ownership manifest contains escaped newlines" "$1"' _ "$HOST"
