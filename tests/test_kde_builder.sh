...
check 'interrupted KDE builds resume only from validated compose outputs' bash -c 'grep -Fq -- "--resume-artifact" "$1" && grep -Fq "KDE resume ownership manifest is missing" "$1" && grep -Fq "KDE resume work mount is still active" "$1" && grep -Fq "KDE resume ownership manifest contains escaped newlines" "$1"' _ "$HOST"
