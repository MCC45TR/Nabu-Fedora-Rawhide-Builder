...
check 'locale regression repair and driver stack have hard gates' bash -c 'grep -Fq "locale-rpm-files.txt" "$1" && grep -Fq -- "-Va" "$1" && grep -Fq "^missing\\\\s+(/usr/share/locale/.*)" "$1" && grep -Fq "locale_repair_packages" "$1" && grep -Fq "dnf-locale-repair.log" "$1" && grep -Fq "installed locale files remain missing" "$1" && grep -Fq "glibc-all-langpacks" "$1" && grep -Fq "nabu-camera-support iris-vaapi-nabu iio-sensor-proxy-nabu libssc-nabu python3-ssc-nabu" "$1"' _ "$COMPOSE"
