...
LOCALE_HELPER="$ROOT/tools/lib/find-missing-rpm-locales.cpp"
...
check 'offline SELinux helper has valid Python test syntax' \
    python3 -m py_compile "$SELINUX_HELPER"
check 'locale helper has valid C++20 syntax' \
    g++ -std=c++20 -Wall -Wextra -Werror -fsyntax-only "$LOCALE_HELPER"
...
check 'CORE sets nabu hostname and keeps every RPM language in composer and target' \
    bash -c 'grep -Fq "macros.zz-nabu-languages" "$1" && grep -Fq "macros.nabu-languages" "$1" && grep -Fq "find-missing-rpm-locales.cpp" "$1" && grep -Fq "printf '\''nabu\\n'\''" "$1" && grep -Fq "FILEFLAGS:fflags" "$2" && grep -Fq "flags.find('\''g'\'')" "$2"' _ "$COMPOSE" "$LOCALE_HELPER"
