...
check 'workflow is manual-only and rebuilds CORE on native ARM64' \
    bash -c '[[ -f "$1" ]] && grep -Eq "^[[:space:]]*workflow_dispatch:" "$1" && ! grep -Eq "^[[:space:]]*(push|pull_request|schedule):" "$1" && grep -Fq "runs-on: ubuntu-24.04-arm" "$1" && grep -Fq "build-core.sh" "$1" && grep -Fq "build-gnome.sh" "$1"' _ "$WORKFLOW"
